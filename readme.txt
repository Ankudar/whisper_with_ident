...
сам скрипт все также проверяет работает ли код или нет
после перезапуска проверяет файл check, получает ссылку на файл и удаляет его, дальше уже продолжается основной код обработки
