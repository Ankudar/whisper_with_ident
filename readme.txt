upd 11.03.25
скрипт нормально не заработал, пришлось немного изменить логику
сам скрипт все также проверяется работает ли код или нет
после перезапуска проверяется файл check, получаем ссылку на файлу и удаляем его, дальше уже продолжается основной код обработки
...
то проверяет файл check, получает оттуда путь к проблемному аудио файлу и удаляет его, после чего заного запускает основной код обработки.
